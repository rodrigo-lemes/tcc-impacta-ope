package com.ezsystem.controller;
...
import com.ezmalteria.domains.AgendamentoTO;
import com.ezmalteria.domains.ClienteTO;
import com.ezmalteria.domains.DespesasTO;
import com.ezmalteria.domains.FuncionarioTO;
import com.ezmalteria.domains.LancamentoServicoTO;
import com.ezmalteria.domains.ProdutoTO;
import com.ezmalteria.domains.UsuarioTO;
...
public class SystemController {
	...
	public ModelAndView index(AgendamentoTO agendamento, ModelAndView model) {
		...
		try {
			while (produtos.next()) {
				...
				produtoAcabando.setNomeProduto(produtos
						.getString("nomeProduto"));
				produtoAcabando.setMarca(produtos.getString("Marca"));
				produtoAcabando.setCor(produtos.getString("cor"));
				produtoAcabando.setCodigoProduto(produtos
						.getString("codigoProduto"));
		...
		try {
			while (agendamentos.next()) {
				...
				agendamento.setData(dateTool.converterDataUsParaBr(agendamentos
						.getString("data")));
				agendamento.setCliente(agendamentos.getString("cliente"));
				agendamento.setFuncionario(agendamentos
				...
				agendamento.setIdAgendamento(agendamentos
						.getString("idAgendamento"));
			...
			if (resultadosDadosUsuario.get(3).equals("adm")
				...
				try {
					...
					while (getNomeFuncionarios.next()) {
						...
						nomesFuncionarios.add(getNomeFuncionarios
								.getString("nome"));
					}
				} catch (SQLException e) {
	...
	public ModelAndView salvarAgendamento(AgendamentoTO agendamento,
			ModelAndView model) {
		...
		if (!loginCheckedContext.equals("erroLogin")) {
			...
			if (insertResult.equals("1")) {
				...
				model = new ModelAndView("gerenciarAgendamentos", "command",
						new AgendamentoTO());
			...
			return model = new ModelAndView(loginCheckedContext, "command",
					null);
		}
	...
	public String formularioDeEdicaoAgendamento(
			@ModelAttribute("command") AgendamentoTO agendamento, ModelMap model) {
		...
		try {
			while (dadosEditarAgendamento.next()) {
				agendamento.setCliente(dadosEditarAgendamento
						.getString("cliente"));
				agendamento.setData(dateTools
						.converterDataUsParaBr(dadosEditarAgendamento
								.getString("data")));
				agendamento.setFuncionario(dadosEditarAgendamento
						.getString("funcionario"));
				agendamento.setHora(dadosEditarAgendamento.getString("hora"));
				agendamento.setServico(dadosEditarAgendamento
						.getString("servico"));
				agendamento.setIdAgendamento(dadosEditarAgendamento
						.getString("idAgendamento"));
			}
		} catch (SQLException e) {
	...
	public ModelAndView salvarAgendamentoEditado(AgendamentoTO agendamento,
			ModelAndView model) {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.AgendamentoTeste SET"
							...
							+ dateTool.converterDataBrParaUS(agendamento
									.getData()) + "', hora='"
							+ agendamento.getHora() + "', cliente='"
							+ agendamento.getCliente() + "', funcionario='"
							+ agendamento.getFuncionario() + "', servico='"
							+ agendamento.getServico()
							+ "'  WHERE idAgendamento='"
							+ codigoEmAgendamentoSendoEditado + "'");
			if (insertResult.equals("1")) {
				model = new ModelAndView("formularioDeEdicaoAgendamento",
						"command", new AgendamentoTO());
	...
	public ModelAndView deletarAgendamento(AgendamentoTO agendamento,
			ModelAndView model) {
	...
	public ModelAndView salvarCliente(ClienteTO cliente, ModelAndView model) {
		...
		insertResult = dataBaseTools
				.insertJdbc(""
						...
						+ cliente.getEmail()
						+ "','"
						+ dateTool.converterDataBrParaUS(cliente
								.getNascimento()) + "','" + cliente.getSexo()
						+ "','" + cliente.getCpf() + "'," + "NOW())");
		...
		if (!loginCheckedContext.equals("erroLogin")) {
			if (insertResult.equals("1")) {
				...
				model = new ModelAndView("gerenciarClientes", "command",
						new ClienteTO());
	...
	public ModelAndView editarDeletarCliente(Model model) {
		...
		try {
			while (clientes.next()) {
				...
				cliente.setNascimento(dateTool.converterDataUsParaBr(clientes
	...
	public String formularioDeEdicaoCliente(
			@ModelAttribute("command") ClienteTO cliente, ModelMap model) {
		...
		try {
			while (dadosEditarCliente.next()) {
				...
				cliente.setNascimento(dateTools
						.converterDataUsParaBr(dadosEditarCliente
								.getString("dataNascimento")));
	...
	public ModelAndView salvarClienteEditado(ClienteTO cliente,
			ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.cliente SET "
							...
							+ cliente.getSexo() + "', `cpf`='"
							+ cliente.getCpf()
							+ "', `dataAlteracao`= NOW() WHERE idcliente = '"
							+ codigoClienteSendoEditado + "'"

					);
			if (insertResult.equals("1")) {
				model = new ModelAndView("formularioDeEdicaoCliente",
						"command", new ClienteTO());
	...
	public ModelAndView relatorioClientes(
			@ModelAttribute("command") ClienteTO cliente) {
		...
		try {
			while (clientes.next()) {
				...
				cliente.setDataInclusao(dateTool.converterDataUsParaBr(clientes
						.getString("dataInclusao")));
			...
			model.addObject("dataPesquisaInicio",
					cliente.getDataPesquisaInicio());
	...
	public ModelAndView salvarFuncionario(FuncionarioTO funcionario,
			ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.insertJdbc("INSERT INTO ezmalteria.funcionario"
							...
							+ " `bairro`," + " `cidade`, " + "	`uf`,"
							+ " `cep`," + " `complemento`,"
							+ " `tipoFuncionario`," + " `carteiraTrabalho`,"
							+ " `statusFuncionario`," + " `dataInclusao`,"
							+ " `codigoFuncionario`," + " `salario`,"
							+ " `email`," + " `senha`," + " `perfil`,"
							+ " `sexo`) VALUES ('"
							+ funcionario.getNome()
							+ "','"
							...
							+ funcionario.getCpf()
							+ "','"
							+ dateTool.converterDataBrParaUS(funcionario
									.getNascimento())
							+ "','"
							+ funcionario.getTelRes()
							+ "','"
							+ funcionario.getTelCel()
							+ "','"
			...
			model = new ModelAndView("gerenciarFuncionarios", "command",
					new FuncionarioTO());
			model.addObject("insertResult", insertResultModelAttribute);
			insertResult = "";
			return model;
	...
	public String formularioDeEdicaoFuncionario(
			@ModelAttribute("command") FuncionarioTO funcionario, ModelMap model) {
		...
		try {
			while (dadosEditarFuncionario.next()) {
				...
				funcionario.setEstadoCivil(dadosEditarFuncionario
				...
				funcionario.setNascimento(dateTools
						.converterDataUsParaBr(dadosEditarFuncionario
								.getString("dataNascimento")));
				funcionario.setTelRes(dadosEditarFuncionario
						.getString("telefone"));
				funcionario.setTelCel(dadosEditarFuncionario
						.getString("celular"));
				funcionario.setLogradouro(dadosEditarFuncionario
						.getString("logradouro"));
				funcionario.setNumero(dadosEditarFuncionario
						.getString("numero"));
				funcionario.setBairro(dadosEditarFuncionario
						.getString("bairro"));
				funcionario.setCidade(dadosEditarFuncionario
						.getString("cidade"));
				funcionario.setUf(dadosEditarFuncionario.getString("uf"));
				funcionario.setCep(dadosEditarFuncionario.getString("cep"));
				funcionario.setComplemento(dadosEditarFuncionario
						.getString("complemento"));
				funcionario.setTipoFuncionario(dadosEditarFuncionario
						.getString("tipoFuncionario"));
				funcionario.setCarteiraTrabalho(dadosEditarFuncionario
						.getString("carteiraTrabalho"));
				funcionario.setDataInclusao(dateTools
						.converterDataUsParaBr(dadosEditarFuncionario
								.getString("dataInclusao")));
				funcionario.setSexo(dadosEditarFuncionario.getString("sexo"));
				funcionario.setCodFunc(dadosEditarFuncionario
						.getString("codigoFuncionario"));
				funcionario.setEstado(dadosEditarFuncionario
						.getString("statusFuncionario"));
				funcionario.setSalarioFixo(dadosEditarFuncionario
						.getString("salario"));
				funcionario.setEmail(dadosEditarFuncionario.getString("email"));
				funcionario.setSenha(dadosEditarFuncionario.getString("senha"));
				funcionario.setPerfil(dadosEditarFuncionario
						.getString("perfil"));
		...
		model.addAttribute("carteiraTrabalho",
				funcionario.getCarteiraTrabalho());
	...
	public ModelAndView salvarFuncionarioEditado(FuncionarioTO funcionario,
			ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.funcionario SET"
							...
							+ funcionario.getSexo()
							+ "'  WHERE cpf='"
							+ cpfCadastradoEmUsuarioSendoEditado + "'");
			...
			if (insertResult.equals("1")) {
				...
				System.out
						.println("Criando modelo Ststus 1 insertResult - EDT_FUNC: "
								+ insertResult);

				model = new ModelAndView("formularioDeEdicaoFuncionario",
						"command", new FuncionarioTO());
				...
				System.out
						.println("Criando modelo Ststus x insertResult - EDT_FUNC: "
								+ insertResult);
				model = new ModelAndView("formularioDeEdicaoFuncionario",
						"command", new FuncionarioTO());
			...
			System.out
					.println("Criando modelo Ststus 1 - EXCEPTION insertResult - EDT_FUNC: "
							+ insertResult);
	...
	public ModelAndView deletarFuncionario(FuncionarioTO funcionario,
			ModelAndView model) {
	...
	public ModelAndView relatorioFuncionarios(
			@ModelAttribute("command") FuncionarioTO funcionario) {
		...
		try {
			while (funcionarios.next()) {
				...
				funcionario.setTipoFuncionario(funcionarios
						.getString("tipoFuncionario"));
				funcionario.setCodFunc(funcionarios
						.getString("codigoFuncionario"));
				funcionario.setEstado(funcionarios
						.getString("statusFuncionario"));

				funcionario.setDataInclusao(dateTool
						.converterDataUsParaBr(funcionarios
								.getString("dataInclusao")));
				...
				System.out.println("Banco "
						+ funcionarios.getString("statusFuncionario"));
			...
			model.addObject("dataPesquisaInicio",
					funcionario.getDataPesquisaInicio());
		...
		try {
			...
			insertResult = dataBaseTools
					.insertJdbc("INSERT INTO ezmalteria.despesa (`codigo`, `tipoDespesa`,"
							...
							+ despesa.getValor()
							+ "','"
							+ dateTool.converterDataBrParaUS(despesa
									.getDataInclusao()) + "')");

			if (insertResult.equals("1")) {
				String insertResultModelAttribute = insertResult;

				model = new ModelAndView("gerenciarDespesas", "command",
						new DespesasTO());
	...
	public String formularioDeEdicaoDespesa(
			@ModelAttribute("command") DespesasTO despesa, ModelMap model) {
		...
		try {
			while (dadosEditarDespesa.next()) {
				...
				despesa.setTipoDespesa(dadosEditarDespesa
						.getString("tipoDespesa"));
				despesa.setValor(dadosEditarDespesa.getString("valor"));
				despesa.setDataInclusao(dateTools
						.converterDataUsParaBr(dadosEditarDespesa
								.getString("dataInclusao")));
	...
	public ModelAndView salvarDespesaEditada(DespesasTO despesa,
			ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.despesa SET codigo='"
							...
							+ "',dataInclusao= '"
							+ dateTool.converterDataBrParaUS(despesa
									.getDataInclusao()) + "' WHERE codigo='"
							+ codigoEmDespesaSendoEditado + "'");

			if (insertResult.equals("1")) {
				model = new ModelAndView("formularioDeEdicaoDespesa",
						"command", new DespesasTO());
	...
	public ModelAndView relatorioDespesas(
			@ModelAttribute("command") DespesasTO despesa) {
		...
		try {
			while (despesas.next()) {
				...
				despesa.setDataInclusao(dateTool.converterDataUsParaBr(despesas
						.getString("dataInclusao")));
		...
		if (!loginCheckedContext.equals("erroLogin")) {
			if ((resultadosDadosUsuario.get(3).equals("adm")
				...
				ModelAndView model = new ModelAndView(
						"gerenciarServicosRealizados", "command",
	...
	public ModelAndView salvarServicoRealizado(
			LancamentoServicoTO registroServicoLancado, ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.insertJdbc("INSERT INTO ezmalteria.lancamentoOrdemServico"
							...
							+ " `formaPagamento`, `nomeFuncionario`) "
							+ "VALUES('"
							+ registroServicoLancado.getNomeCliente()
							+ "','"
							+ registroServicoLancado.getComanda()
							+ "','"
							+ registroServicoLancado.getAlicate()
							+ "','"
							+ registroServicoLancado.getTipoFuncionario()
							+ "','"
							+ dateTool
									.converterDataBrParaUS(registroServicoLancado
											.getDataInclusao())
							+ "','"
							+ registroServicoLancado.getManicurePreco()
							+ "','"
							+ registroServicoLancado.getPedicurePreco()
							+ "','"
							+ registroServicoLancado.getFrancesinhaPreco()
							+ "','"
							+ registroServicoLancado.getEsmaltarPreco()
							+ "','"
							+ registroServicoLancado.getPernaintPreco()
							+ "','"
							+ registroServicoLancado.getPerna12Preco()
							+ "','"
							+ registroServicoLancado.getVirilhaPreco()
							+ "','"
							+ registroServicoLancado.getIntimaPreco()
							+ "','"
							+ registroServicoLancado.getAxilaPreco()
							+ "','"
							+ registroServicoLancado.getSobrancelhaPreco()
							+ "','"
							+ registroServicoLancado.getBucoPreco()
							+ "','"
							+ registroServicoLancado.getPreco()
							+ "','"
							+ registroServicoLancado.getFormaPagamento()
							+ "','"
							+ registroServicoLancado.getNomeFuncionario()
							+ "')");

			if (insertResult.equals("1")) {
				String insertResultModelAttribute = insertResult;

				model = new ModelAndView("gerenciarServicosRealizados",
			...
			model = new ModelAndView("gerenciarServicosRealizados", "command",
					new LancamentoServicoTO());
	...
	public ModelAndView editarDeletarServicoRealizado(Model model) {
		...
		try {
			while (servicosPrestados.next()) {
				...
				servico.setIdLancamento(servicosPrestados
						.getString("idLancamento"));

				servico.setNomeCliente(servicosPrestados
						.getString("nomeCliente"));

				servico.setComanda(servicosPrestados.getString("comanda"));

				servico.setAlicate(servicosPrestados.getString("alicate"));

				servico.setTipoFuncionario(servicosPrestados
						.getString("tipoFuncionario"));

				servico.setDataInclusao(dateTool
						.converterDataUsParaBr(servicosPrestados
								.getString("dataInclusao")));

				servico.setManicurePreco(servicosPrestados
						.getString("manicurePreco"));

				servico.setPedicurePreco(servicosPrestados
						.getString("pedicurePreco"));

				servico.setFrancesinhaPreco(servicosPrestados
						.getString("francesinhaPreco"));

				servico.setEsmaltarPreco(servicosPrestados
						.getString("esmaltarPreco"));

				servico.setPernaintPreco(servicosPrestados
						.getString("pernaintPreco"));

				servico.setPerna12Preco(servicosPrestados
						.getString("perna12Preco"));

				servico.setVirilhaPreco(servicosPrestados
						.getString("virilhaPreco"));

				servico.setIntimaPreco(servicosPrestados
						.getString("intimaPreco"));

				servico.setAxilaPreco(servicosPrestados.getString("axilaPreco"));

				servico.setSobrancelhaPreco(servicosPrestados
						.getString("sobrancelhaPreco"));

				servico.setBucoPreco(servicosPrestados.getString("bucoPreco"));

				servico.setPreco(servicosPrestados.getString("preco"));

				servico.setFormaPagamento(servicosPrestados
						.getString("formaPagamento"));

				servico.setNomeFuncionario(servicosPrestados
						.getString("nomeFuncionario"));
			...
			System.out.println("primeiro item da lista: "
					+ listaServicos.get(0).getIdLancamento());
	...
	public String formularioDeEdicaoServicosRealizados(
			@ModelAttribute("command") LancamentoServicoTO servicos,
			ModelMap model) {
		...
		try {
			while (servicosPrestados.next()) {
				...
				servicos.setIdLancamento(servicosPrestados
						.getString("idLancamento"));

				servicos.setNomeCliente(servicosPrestados
						.getString("nomeCliente"));
				...
				servicos.setTipoFuncionario(servicosPrestados
						.getString("tipoFuncionario"));

				servicos.setDataInclusao(dateTools
						.converterDataUsParaBr(servicosPrestados
								.getString("dataInclusao")));

				servicos.setManicurePreco(servicosPrestados
						.getString("manicurePreco"));

				servicos.setPedicurePreco(servicosPrestados
						.getString("pedicurePreco"));

				servicos.setFrancesinhaPreco(servicosPrestados
						.getString("francesinhaPreco"));

				servicos.setEsmaltarPreco(servicosPrestados
						.getString("esmaltarPreco"));

				servicos.setPernaintPreco(servicosPrestados
						.getString("pernaintPreco"));

				servicos.setPerna12Preco(servicosPrestados
						.getString("perna12Preco"));

				servicos.setVirilhaPreco(servicosPrestados
						.getString("virilhaPreco"));

				servicos.setIntimaPreco(servicosPrestados
						.getString("intimaPreco"));

				servicos.setAxilaPreco(servicosPrestados
						.getString("axilaPreco"));

				servicos.setSobrancelhaPreco(servicosPrestados
						.getString("sobrancelhaPreco"));
				...
				servicos.setFormaPagamento(servicosPrestados
						.getString("formaPagamento"));

				servicos.setNomeFuncionario(servicosPrestados
						.getString("nomeFuncionario"));
	...
	public ModelAndView salvarEdicaoServicoRealizado(
			LancamentoServicoTO servico, ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.lancamentoOrdemServico SET nomeCliente='"
							...
							+ servico.getManicurePreco()
							+ "', pedicurePreco='"

							+ servico.getPedicurePreco()
							+ "', francesinhaPreco='"

							+ servico.getFrancesinhaPreco()
							+ "', esmaltarPreco='"

							+ servico.getEsmaltarPreco()
							+ "', pernaintPreco='"

							+ servico.getPernaintPreco()
							+ "', perna12Preco='"

							+ servico.getPerna12Preco()
							+ "', virilhaPreco='"

							+ servico.getVirilhaPreco()
							+ "', intimaPreco='"

							+ servico.getIntimaPreco()
							+ "', axilaPreco='"

							+ servico.getAxilaPreco()
							+ "', sobrancelhaPreco='"

							+ servico.getSobrancelhaPreco()
							+ "', bucoPreco='"

							+ servico.getBucoPreco()
							+ "', preco='"
			...
			if (insertResult.equals("1")) {
				model = new ModelAndView(
						"formularioDeEdicaoServicosRealizados", "command",
						new LancamentoServicoTO());
	...
	public ModelAndView deletarServicoRealizado(LancamentoServicoTO servico,
			ModelAndView model) {
	...
	public ModelAndView relatorioServicosRealizados(
			@ModelAttribute("command") LancamentoServicoTO servico) {
		...
		ResultSet servicosPrestados = dataBaseTools
				.selectJdbc("SELECT * FROM ezmalteria.lancamentoOrdemServico WHERE dataInclusao != '0001-01-01"
						...
						+ servico.getNomePesquisaCliente()
						+ "%' AND nomeFuncionario like '%"
						+ servico.getNomePesquisaFuncionario()
		...
		try {
			...
			model.addObject("dataPesquisaInicio",
					servico.getDataPesquisaInicio());
			model.addObject("dataPesquisaFim", servico.getDataPesquisaFim());
			model.addObject("nomePesquisaFuncionario",
					servico.getNomePesquisaFuncionario());
			model.addObject("nomePesquisaCliente",
					servico.getNomePesquisaCliente());
		} catch (SQLException e) {
			log.gravarLog(e.toString());
		}
		...
		try {
			...
			insertResult = dataBaseTools
					.insertJdbc("INSERT INTO ezmalteria.produto (`tipo`,"
							...
							+ produto.getMarca() + "','" + produto.getCor()
			...
			if (insertResult.equals("1")) {
				...
				model = new ModelAndView("gerenciarProdutos", "command",
						new ProdutoTO());
	...
	public String formularioDeEdicaoProduto(
			@ModelAttribute("command") ProdutoTO produto, ModelMap model) {
		...
		try {
			while (dadosEditarProduto.next()) {
				produto.setNomeProduto(dadosEditarProduto
						.getString("nomeProduto"));
				...
				produto.setQuantidade(dadosEditarProduto
						.getString("quantidade"));
				produto.setValor(dadosEditarProduto.getString("valor"));
				produto.setCodigoProduto(dadosEditarProduto
						.getString("codigoProduto"));
				produto.setDataInclusao(dateTools
						.converterDataUsParaBr(dadosEditarProduto
								.getString("dataInclusao")));

				System.out.println(produto.getNomeProduto() + ","
						+ produto.getTipo() + "," + produto.getMarca() + ","
						+ produto.getCor() + "," + produto.getQuantidade()
						+ "," + produto.getValor() + ","
						+ produto.getCodigoProduto() + ","
	...
	public ModelAndView salvarProdutoEditado(ProdutoTO produto,
			ModelAndView model) {
		...
		try {
			...
			insertResult = dataBaseTools
					.updateJdbc("UPDATE ezmalteria.produto SET" + " `tipo`='"
							...
							+ produto.getCor() + "',`quantidade`='"
							+ produto.getQuantidade() + "',`valor`='"
							+ produto.getValor() + "',`codigoProduto`='"
							+ produto.getCodigoProduto()
							+ "' WHERE codigoProduto='"
			...
			if (insertResult.equals("1")) {
				...
				model = new ModelAndView("formularioDeEdicaoProduto",
						"command", new ProdutoTO());
			...
			model = new ModelAndView("formularioDeEdicaoProduto", "command",
					new ProdutoTO());
	...
	public ModelAndView recuperarSenha() {
		return new ModelAndView("recuperarSenha", "command",
				new FuncionarioTO());
	}
	...
	public String enviarEmailSenha(FuncionarioTO funcionario) {
		...
		if (!senha.isEmpty()) {
			JMailTool.sendEmail("ezspwdrecovery@gmail.com", "ccx13980",
					funcionario.getEmail(),
					"Serviço para recuperação de senha.\n\nSenha Atual: "
							+ senha, "solicitacao de envio de senha");
	...
	public String relatorioFinanceiro(Model modelo,
			@ModelAttribute("command") DespesasTO despesa) {
		// Entidades de busca
		...
		try {
			while (despesasCadastradas.next()) {
				...
				despesa.setDataInclusao(dateTool
						.converterDataUsParaBr(despesasCadastradas
								.getString("dataInclusao")));
		...
		try {
			while (servicosLancados.next()) {
				...
				servicoLancado.setDataInclusao(dateTool
						.converterDataUsParaBr(servicosLancados
								.getString("dataInclusao")));

				servicoLancado.setAxilaPreco(servicosLancados
						.getString("axilaPreco"));
				servicoLancado.setBucoPreco(servicosLancados
						.getString("bucoPreco"));
				servicoLancado.setEsmaltarPreco(servicosLancados
						.getString("esmaltarPreco"));
				servicoLancado.setFrancesinhaPreco(servicosLancados
						.getString("francesinhaPreco"));
				servicoLancado.setIntimaPreco(servicosLancados
						.getString("intimaPreco"));
				servicoLancado.setManicurePreco(servicosLancados
						.getString("manicurePreco"));
				servicoLancado.setPedicurePreco(servicosLancados
						.getString("pedicurePreco"));
				servicoLancado.setPerna12Preco(servicosLancados
						.getString("perna12Preco"));
				servicoLancado.setPernaintPreco(servicosLancados
						.getString("pernaintPreco"));
				servicoLancado.setSobrancelhaPreco(servicosLancados
						.getString("sobrancelhaPreco"));
				servicoLancado.setVirilhaPreco(servicosLancados
						.getString("virilhaPreco"));
